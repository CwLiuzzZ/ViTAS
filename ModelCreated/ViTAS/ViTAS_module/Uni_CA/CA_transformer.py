...
from ModelCreated.ViTAS.ViTAS_module.Uni_CA.attention import (single_head_full_attention, single_head_split_window_attention,
                        single_head_full_attention_1d, single_head_split_window_attention_1d)
from ModelCreated.ViTAS.ViTAS_module.Uni_CA.CA_utils import generate_shift_window_attn_mask, generate_shift_window_attn_mask_1d
...
class TransformerLayer(nn.Module):
    def __init__(self,
        ...
        if input_d_model is None:
            input_d_model = d_model
        ...
        self.k_proj = nn.Linear(input_d_model, d_model, bias=False)
        self.v_proj = nn.Linear(input_d_model, d_model, bias=False)
    ...
    def forward(self, source, target,
        ...
        # attn_type = self_swin2d_cross_swin1d, is_self_attn = True for self-attention, attn_num_splits = 2
        # attn_type = self_swin2d_cross_swin1d, is_self_attn = False for cross-attention, attn_num_splits = 2
...
class FeatureFuseTransformer(nn.Module):
    def __init__(self,
                 num_layers=6,
                 d_model=128,
                 nhead=1,
        ...
        super(FeatureFuseTransformer, self).__init__()

        self.d_model = d_model
        self.nhead = nhead

        self.layer = TransformerBlock(d_model=d_model,
                             nhead=nhead,
                             ffn_dim_expansion=ffn_dim_expansion,
                             input_d_model = input_d_model
                             )
    ...
    def forward(self, feature0, feature1,
        ...
        fused = self.layer(feature0, feature1,
                        height=h,
                        width=w,
                        attn_type=attn_type,
                        with_shift='swin' in attn_type and attn_num_splits > 1 and 0 % 2 == 1,
                        attn_num_splits=attn_num_splits,
                        shifted_window_attn_mask=shifted_window_attn_mask,
                        shifted_window_attn_mask_1d=shifted_window_attn_mask_1d,
                        )
        ...
        # reshape back
        fused = fused.view(b, h, w, c).permute(0, 3, 1, 2).contiguous()  # [B, C, H, W]
