...
from toolkit.function.base_function import save_disp_results,io_disp_read
...
def validate_kitti(model, iters=32, mixed_prec=False):
    ...
    model.eval()
    aug_params = {}
    val_dataset = datasets.KITTI(aug_params, image_set='testing')
    torch.backends.cudnn.benchmark = True

    out_list, epe_list, elapsed_list = [], [], []
    for val_id in tqdm(range(len(val_dataset))):
        dirs, image1, image2, flow_gt, valid_gt = val_dataset[val_id]
        image1 = image1[None].cuda()
        image2 = image2[None].cuda()
        ...
        flow_pr = flow_pr[0].detach().cpu().numpy()
        
        save_method = 'IGEV'
        split_path = dirs[0].split('/')
        ...
        gt_dir = os.path.join(save_dir,'IGEV_gt/disp/{}.npy'.format(name))
        gt_disp = io_disp_read(gt_dir)
        ...
        # max_disp = np.max(flow_pr[int(flow_pr.shape[0]/2):,3:flow_pr.shape[1]-3])+5
        # save_disp_results(save_path1,save_path2,flow_pr,max_disp,display=False)
        # flow_pr[0,-1] = np.max(flow_pr)+5
        # save_disp_results(save_path1,save_path2,flow_pr,display=False)
        save_disp_results(save_path1,save_path2,flow_pr,max_disp=np.max(gt_disp),min_disp=0,display=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore_ckpt', help="restore checkpoint", default='./pretrained_models/kitti15.pth')
